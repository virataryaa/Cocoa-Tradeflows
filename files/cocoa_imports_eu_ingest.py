"""
Hardmine — TDM Cocoa EU Imports Ingest
=======================================
Usage:
    python cocoa_imports_eu_ingest.py            # incremental
    python cocoa_imports_eu_ingest.py --full     # full history from 201501

NOTE: API key auth + E28 aggregate reporter.
Saves to: .../Cocoa Flows/files/data/tdm_cocoa_imports_eu.parquet
"""
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(LOG_DIR / "cocoa_imports_eu_ingest.log", encoding="utf-8"),
    ],
)
...
OUT_FILE   = Path(__file__).parent / "data" / "tdm_cocoa_imports_eu.parquet"
...
REPORTER_REGION = {"EU 28 External Trade": "Europe"}

PARTNER_FIX = {
    "United States":"United States of America","Russia":"Russian Federation",
    "South Korea":"Korea, Republic of","Cote d'Ivoire":"Cote d'Ivoire",
    "Congo (ROC)":"Congo","Congo (DROC)":"Democratic Republic of the Congo",
    "Netherlands Antilles":"Other","Duty Free Shops":"Other",
    "Stores and Provisions":"Other","Free Zones":"Other",
    "High Seas":"Other","Unidentified":"Other",
}
...
def main():
    ...
    log.info("=" * 60)
    log.info("Cocoa EU Imports Ingest  |  %s", datetime.now().strftime("%Y-%m-%d %H:%M"))
