"""
Cocoa TDM Trade Flow Dashboard
================================
...
st.set_page_config(
    page_title="Cocoa TDM Trade Flow Dashboard",
    layout="wide",
...
# ── Sidebar ───────────────────────────────────────────────────────────────────
with st.sidebar:
    st.markdown("<div class='sb-title'>Cocoa Trade Flow</div>", unsafe_allow_html=True)
    st.markdown("<div class='sb-caption'>Cocoa exports and imports by country, destination and crop year.</div>", unsafe_allow_html=True)
    _CY_PRESETS  = ["Oct\u2013Sep", "Jan\u2013Dec", "Custom"]
    ...
    if _cy_basis == "Oct\u2013Sep":
        crop_start_month = 10
    elif _cy_basis == "Jan\u2013Dec":
        crop_start_month = 1
    else:
        _cs = st.selectbox("Start month", _MONTH_ABBRS, index=9, key="cy_custom_start")
        crop_start_month = _MONTH_ABBRS.index(_cs) + 1
    st.markdown("---")
    unit_choice = st.radio(
        "Unit",
        ["k MT (BEQ)", "k MT (Raw)", "Lots (BEQ)", "Lots (Raw)"],
        index=0, key="unit_choice_global",
    )

# ── Derived unit format (all units have large numbers — always use commas) ────
_num_fmt  = ",.1f"
_TC       = "#1d1d1f"
_GC       = "#f0f0f0"
_TMPL     = "plotly_white"
...
FLOW_PATHS = {
    "Cocoa Exports":           str(_DATA / "tdm_cocoa_exports.parquet"),
    "Cocoa Imports":           str(_DATA / "tdm_cocoa_imports.parquet"),
...
with tab1:
    ...
    _rep_default_sf = (
        [r for r in ["Cote d'Ivoire"] if r in _all_reps_sf] if _is_exports
        else [r for r in ["United States"] if r in _all_reps_sf]
    ) or _all_reps_sf
    ...
    with st_ov:
        with st.expander("Filters", expanded=False):
            ...
            with fc1:
                ...
                if f"{_sf}_reporters" not in st.session_state:
                    _sf_rep = (
                        [r for r in ["Cote d'Ivoire"] if r in reporters_in_scope] if _is_exports
                        else [r for r in ["United States"] if r in reporters_in_scope]
        ...
        # ── HTML heatmap (avoids Streamlit/Styler "None" rendering issues) ────
        ...
        # ── Rolling chart (full width — YTD now in heatmap) ────────────────────
    ...
    with st_dest:
        ...
        with dest_fc2:
            ...
            if f"{_sf}_reporters" not in st.session_state:
                _dd_rep_sf = (
                    [r for r in ["Cote d'Ivoire"] if r in _dest_reporters_scope] if _is_exports
